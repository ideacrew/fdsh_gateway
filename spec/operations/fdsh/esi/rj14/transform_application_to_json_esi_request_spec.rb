...
RSpec.describe Fdsh::Esi::Rj14::TransformApplicationToJsonEsiRequest, dbclean: :after_each do
  include_context "application hash for cv3"

  it 'converts the application hash for esi mec hub call' do
    result = subject.call(application_params.to_json)
    expect(result).to be_success
    applicant = result.success[:esiMECRequest][:applicantRequestArray].first
