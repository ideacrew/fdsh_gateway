...
RSpec.describe Fdsh::Jobs::Vlp::GenerateTransmittableInitialVerificationPayload, dbclean: :after_each do
  include_context "person hash for cv3"

  subject { described_class.new }
  let(:key) { :vlp_verification_request}
  let(:title) { 'VLP Verification Request'}
  let(:description) { 'Request for VLP verification to CMS'}
