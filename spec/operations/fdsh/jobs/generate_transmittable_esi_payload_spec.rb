...
RSpec.describe Fdsh::Jobs::GenerateTransmittableEsiPayload, dbclean: :after_each do
  include_context "application hash for cv3"

  subject { described_class.new }
  let(:key) { :esi_mec_request}
  let(:title) { 'Esi Mec Request' }
  let(:description) { 'Request for esi mec for CMS' }
    ...
    let(:json_payload) do
      { :esiMECRequest =>
        { :applicantRequestArray =>
          [{ :personSurName => "evidence", :personGivenName => "esi", :personBirthDate => "1988-11-11", :personSexCode => "M",
             :personSocialSecurityNumber => "518124854", :usStateCode => "DC",
             :requestedCoveragePeriod => { :startDate => "2021-01-01", :endDate => "2021-12-31" } }] } }
    end
