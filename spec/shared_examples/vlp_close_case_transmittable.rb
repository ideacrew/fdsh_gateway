...
RSpec.shared_context 'vlp close case transmittable job transmission transaction', shared_context: :metadata do
  include_context "person hash for cv3"
  let(:payload) { person_params.to_json }

  let!(:job) do
    job = FactoryBot.create(:transmittable_job, key: :vlp_close_case_request, started_at: DateTime.now)
  ...
  let!(:transmission) do
    transmission = FactoryBot.create(:transmittable_transmission, job: job, key: :vlp_close_case_request, started_at: DateTime.now)
  ...
  let!(:transaction) do
    transaction = person_subject.transactions.create(key: :vlp_close_case_request, started_at: DateTime.now, xml_payload: "A REQUEST PAYLOAD")
