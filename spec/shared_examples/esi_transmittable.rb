...
RSpec.shared_context 'esi transmittable job transmission transaction', shared_context: :metadata do
  include_context "application hash for cv3"
  let(:payload) { application_params.to_json }

  let!(:job) do
    job = FactoryBot.create(:transmittable_job, key: :esi_mec_request)
  ...
  let!(:transmission) do
    transmission = FactoryBot.create(:transmittable_transmission, job: job, key: :esi_mec_request)
  ...
  let!(:transaction) do
    transaction = person_subject.transactions.create(key: :esi_mec_request, started_at: DateTime.now, json_payload: JSON.parse(payload))
