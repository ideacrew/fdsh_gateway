  ...
  module Jobs
    ...
    class GenerateTransmittableNonEsiPayload
      ...
      def create_application_subject(values)
        existing_application = ::Transmittable::Application.where(hbx_id: values[:correlation_id]).last
        ...
        if existing_application
          ...
          application = ::Transmittable::Application.create(hbx_id: values[:correlation_id],
      ...
      def generate_transmittable_payload(payload)
        result = ::Fdsh::NonEsi::Rj31::TransformApplicationToJsonNonEsiRequest.new.call(payload)
        if result.success?
